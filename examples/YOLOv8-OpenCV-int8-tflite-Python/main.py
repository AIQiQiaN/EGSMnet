...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        """Return updated annotations and image with added border."""
...
class Yolov8TFLite:
    ...
    def draw_detections(self, img, box, score, class_id):
        """
        Draws bounding boxes and annotations on the input image based on the detected objects.
        ...
        # Create the labels text with class name and score
        label = f"{self.classes[class_id]}: {score:.2f}"

        # Calculate the dimensions of the labels text
        (label_width, label_height), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)

        # Calculate the position of the labels text
        label_x = x1
        label_y = y1 - 10 if y1 - 10 > label_height else y1 + 10

        # Draw a filled rectangle as the background for the labels text
        cv2.rectangle(
            img,
        ...
        # Draw the labels text on the image
