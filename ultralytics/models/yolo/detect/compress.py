...
from ultralytics.nn.modules import Detect, v10Detect, RepConv, PSA
...
# 没有购买yolov8项目需要注释以下
from ultralytics.nn.extra_modules import Detect_Efficient, Detect_DyHead_Prune, Detect_LSCD
from ultralytics.nn.extra_modules.block import Faster_Block, Fusion, IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, SimFusion_4in, AdvPoolFusion, PyramidPoolAgg, RepVGGBlock, RepConvN, Star_Block
...
def get_pruner(opt, model, example_inputs):
    ...
    #     if isinstance(m, RepVGGBlock): # 视频中忘记加了
    ...
    #         ignored_layers.append(m.cv3[1][2])
    #         ignored_layers.append(m.cv3[2][2])
    #         ignored_layers.append(m.one2one_cv2[0][2])
    #         ignored_layers.append(m.one2one_cv2[1][2])
    #         ignored_layers.append(m.one2one_cv2[2][2])
    #         ignored_layers.append(m.one2one_cv3[0][2])
    #         ignored_layers.append(m.one2one_cv3[1][2])
    #         ignored_layers.append(m.one2one_cv3[2][2])
    #         ignored_layers.append(m.dfl)
    #     if isinstance(m, PSA):
    #         ignored_layers.append(m)
    ...
    pruner = pruner_entry(
        ...
        # root_module_types=[nn.Conv2d, nn.Linear, DyHeadBlock_Prune]
        # root_module_types=[nn.Conv2d, nn.Linear, RepConvN]
    )
