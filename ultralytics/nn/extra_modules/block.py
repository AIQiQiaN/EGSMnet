...
#     from mmengine.model import constant_init, normal_init
#     MMCV_AVAILABLE = True
#     print("✓ MMCV modules loaded successfully")
# except ImportError as e:
#     MMCV_AVAILABLE = False
#     print(f"✗ MMCV import failed: {e}")
#     print("Attempting to use fallback implementations...")
...
class EASA(nn.Module):
    def __init__(self, dim=36, down_scale=1):
        super(EASA, self).__init__()
        self.linear_1 = nn.Conv2d(dim, dim, 1, 1, 0)
        self.linear_2 = nn.Conv2d(dim, dim, 1, 1, 0)
        self.dw_conv = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
    ...
    def forward(self, X):
        ...
        # 用 max_pool2d 替换 adaptive_max_pool2d，ONNX 完全支持
        # down_scale=1 时 stride=1，池化核=1，等价于不做下采样（最小尺寸保护天然满足）
        stride = max(1, self.down_scale)
        kernel = stride  # kernel==stride 时，输出尺寸 = ceil(h/stride)
        pooled = F.max_pool2d(X, kernel_size=kernel, stride=stride, padding=0)
