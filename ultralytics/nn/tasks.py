...
from ultralytics.nn.dual_stream_modules import *
from ultralytics.nn.new_models import *
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            C2f_SHSA, C2f_SHSA_CGLU, C2f_SMAFB, C2f_SMAFB_CGLU, EASA, EGConv, GhostC2f, C2PSA, C3k2,
        }:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in {
                     BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3k2, C3TR, C3Ghost, C3x, RepC3, C2fCIB, C2f_Faster, C2f_ODConv, C2f_Faster_EMA, C2f_DBB,
                     ...
                     C2f_SHSA, C2f_SHSA_CGLU, C2f_SMAFB, C2f_SMAFB_CGLU, GhostC2f,
                     }:
            ...
            c2 = m.channel
                   ...
                   DySample, CARAFE, CAA, ELA, CAFM, AFGCAttention, EUCB, SAGE,}:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
        ...
        # if m in (WeightedMultiScaleFusion, MultiScaleFeatureFusion):
        #     # 获取多个输入层的通道数
        #     c1_list = [ch[x] for x in f]  # f = [2, 5, 7, 10]
        #     c2 = args[0]  # 输出通道数
        ...
        elif m is InputProxy:
            c2 = ch[f]  # 输出通道与输入相同（6ch 恒等透传）
        elif m is ChannelSplit:
            c2 = args[1] - args[0]  # 输出通道 = end - start（如 3-0=3）
        elif m is CrossModalFusion:
            c2 = args[0]  # 输出通道 = 传入的 c 参数

        else:
            # c2 = ch[f]
            if isinstance(f, list):
                # 已知的多输入模块（Concat、Fusion、ModalityPromptInjection 等）
                if m is Concat or str(m) == "Concat":
                    c2 = sum(ch[x] for x in f)
                elif str(m) in {"ModalityPromptInjection", "MPI"} or (hasattr(m, "__name__") and m.__name__ in {"ModalityPromptInjection", "MPI"}):
                    # ModalityPromptInjection：接收 [RGB, IR]，输出融合后通道数（这里假设输出为 3 通道，可根据你的模块实际输出调整）
                    c2 = 3
                elif str(m) in {"CrossModalGraphReasoning", "TaskGuidedDiffusionPrior", "CMGR", "TGDP"} or \
                     (hasattr(m, "__name__") and m.__name__ in {"CrossModalGraphReasoning", "TaskGuidedDiffusionPrior"}):
                    # CMGR / TGDP：输入输出通道数相同，取 args[0] 作为通道数
                    c2 = args[0] if args else ch[f[0]]
                elif m is InputProxy or str(m) == "InputProxy":
                    c2 = ch[f] if isinstance(f, int) else ch[f[0]]
                elif m is ChannelSplit or str(m) == "ChannelSplit":
                    c2 = args[1] - args[0]   # end - start
                else:
                    # 其他多输入模块默认求和（兼容 Concat 类行为）
                    c2 = sum(ch[x] for x in f)
            else:
                # 单输入模块走原逻辑
                c2 = ch[f]
